        ...
        return {
            ...
            async createFreshOrder(customer = null) {
                ...
                if (currentOrder?.isEmpty?.()) {
                    ...
                    this.pos.remove_order?.(currentOrder);
                }
                ...
                if (!order) {
                    throw new Error(_t("Unable to create a new POS order."));
                }

                /*
                * Important when reopening an existing laundry order:
                * explicitly make the newly created order the active POS order.
                */
            ...
            _setLaundryOrderState(order, values = {}) {
                ...
                order.uiState.laundry_order_type_id =
                    values.laundry_order_type_id || false;
            ...
            async openLaundryOrder(orderId) {
                ...
                if (this.isReadOnlyPaymentStatus(data.payment_status_id)) {
                    ...
                    this.pos.navigate("pos_laundrypaidorderscreen", {
                ...
                /*
                * Clear category state left by the previous normal/package order.
                */
                this.pos.selectedCategory = undefined;
                this.pos.selectedCategoryId = undefined;
                this.pos.selected_category = undefined;
                this.pos.selected_category_id = undefined;
                this.pos.productListCategoryId = undefined;

                order.uiState.laundry_start_category_opened = false;
                ...
                for (const line of data.lines || []) {
                    const product = this._getModelRecord(
                        "product.product",
                        line.product_id
                    );
                ...
                console.log("Opening existing laundry order", {
                    requestedLaundryOrderId: orderId,
                    posOrderUuid: order.uuid,
                    activeOrderUuid:
                        this.getOrder()?.uuid,
                    orderTypeId:
                        order.uiState?.laundry_order_type_id,
                    isPackageUsage:
                        Boolean(order.uiState?.is_package_usage),
                    allowedCategoryIds:
                        order.uiState
                            ?.laundry_allowed_pos_category_ids,
                    allowedPackageProducts:
                        order.uiState
